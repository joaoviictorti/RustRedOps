...
type MessageBoxAType = unsafe extern "system" fn(HWND, *const i8, *const i8, u32) -> i32;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    unsafe {
        ...
        let mut user32_dll_unicode = UNICODE_STRING {
            ...
            Buffer: PWSTR(user32_dll_name.as_ptr().cast_mut()),
        };

        // Get user32.dll
        let LdrLoadDll = std::mem::transmute::<_, LdrLoadDllType>(trampoline);
        let mut user32handle = null_mut();
        LdrLoadDll(ptr::null_mut(), 0 as *mut u32, &mut user32_dll_unicode, &mut user32handle);
        ...
        MessageBoxA(HWND(0), c"Hello, World!".as_ptr(), c"Title".as_ptr(), 0);
    }
